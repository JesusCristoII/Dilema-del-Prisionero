...
class Evolution:
    ...
    def natural_selection(self, result_tournament: dict[Player, float]) \
        ...
        Results:
         ...
         dynamics applied
        """
        result_ord = list(sorted(self.ranking.keys(),key=lambda x: self.ranking[x]))
        for i in range(int(len(self.players)*self.reproductivity)):
            result_ord.pop()
        for i in range(int(len(self.players)*self.reproductivity)):
            result_ord.append(type(result_ord[i])(result_ord[i].dilemma))
    ...
    def play(self, do_print: bool = False):
        ...
        for i in range(self.generations):
            torneo = Tournament(self.players,self.n_rounds,self.error,self.repetitions)
            torneo.play()
            self.natural_selection(torneo.ranking)
            if do_print:
                print(self.players)
        if do_print:
            resul = []
            for pl in self.players:
                resul.append((pl,self.players.count(pl)))
            print(set(resul))
